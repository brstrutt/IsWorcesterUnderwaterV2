...
pub fn page() -> Html {
    ...
    html! {
        <>
            if barbourne_river_level.is_some() && diglis_river_level.is_some() {
                <river_level_display::Background
                    barbourne_last_reading={barbourne_river_level.unwrap()}
                    diglis_last_reading={diglis_river_level.unwrap()}
                />
            }
